...
@Service
class PrintScriptServiceImpl(private val redisTemplate: RedisTemplate<String, Any>, private val snippetRepository : SnippetRepository) : PrintScriptService {
    private val objectMapper = jacksonObjectMapper()
    override fun analyzeAllSnippets(scaSnippetRules : SCARulesDTO, userData : Jwt) {
        ...
        for (snippetData in snippets) {
            val scaRulesRedisDTO = SCASnippetWithRulesRedisDTO(
    ...
    override fun formatAllSnippets(formatSnippetRules : FormatRulesDTO, userData : Jwt) {
        val userEmail = userData.claims["email"].toString()
        val snippets = snippetRepository.getSnippetsByAuthor(userEmail)

        for (snippetData in snippets) {
            val formatRulesRedisDTO = FormatSnippetWithRulesRedisDTO(
